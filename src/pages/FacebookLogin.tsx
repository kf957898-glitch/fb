...
const FacebookLogin: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loggedIn = localStorage.getItem('facebookLoggedIn');
    if (loggedIn === 'true') {
      setIsLoggedIn(true);
    }
  }, []);
  ...
  if (isLoggedIn) {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center px-4">
        <div className="bg-white rounded-lg shadow-lg border border-gray-200 px-8 py-6 text-center max-w-md w-full">
          <h1 className="text-2xl font-bold text-green-600 mb-4">Welcome to Facebook!</h1>
          <p className="text-gray-600 mb-6">You have successfully logged in.</p>
          <p className="text-sm text-gray-500 mb-6">Email: {JSON.parse(localStorage.getItem('facebookCredentials') || '{}').email}</p>
          <button
